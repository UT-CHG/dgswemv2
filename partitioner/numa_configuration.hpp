  ...
  public:
    ...
    NumaConfiguration(const std::string& type) {
        if (type.compare("default")) {
            ...
            std::cerr << "Numa Configuration: " << type << " is invalid\n";
            std::cerr << "Aborting\n";
            exit(1);
        }
    }

    inline const std::vector<double>& get_numa_wghts() const { return numa_wghts; }

    inline std::size_t get_num_numa_domains() const { return numa_wghts.size(); }
