...
public:
  ...
  std::unordered_map<int,std::vector<int>> get_edges()
  { return _edges; }
...
std::vector<int64_t> scale_to_int64(const std::vector<T> &vec) {
  double max_aval = 0;
  for (auto val : vec) {
    if (fabs(val) > max_aval) max_aval = fabs(val);
  }
...
get_node_weights(const std::vector<int> &node_ids,
...
std::vector<int64_t> metis_part(const CSRMat<NodeW> &mat, int64_t nparts,
  ...
  METIS_PartGraphKway( &nvtxs, &ncon,
		       to_int64(mat.xadj()).data(),
		       to_int64(mat.adj()).data(),
		       node_wgts.data(), nullptr,
		       edge_wgts.data(),
