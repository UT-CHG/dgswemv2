...
void Problem::global_edge_boundary_kernel(const RKStepper& stepper, EdgeBoundaryType& edge_bound) {
    ...
    while (true) {
        ...
        for (uint dof = 0; dof < edge_bound.edge_data.get_ndof_global(); ++dof) {
...
void Problem::global_edge_boundary_iteration(const RKStepper& stepper, EdgeBoundaryType& edge_bound) {
    ...
    for (uint dof_i = 0; dof_i < edge_bound.edge_data.get_ndof_global(); ++dof_i) {
        for (uint dof_j = 0; dof_j < edge_bound.edge_data.get_ndof_global(); ++dof_j) {
            Matrix<double, 3, 3> A =
                edge_bound.IntegrationLambdaLambda(dof_i, dof_j, edge_global.delta_hat_kernel_at_gp);
        ...
        Vector<double, 3> b = edge_bound.IntegrationLambda(dof_i, edge_global.rhs_kernel_at_gp);
