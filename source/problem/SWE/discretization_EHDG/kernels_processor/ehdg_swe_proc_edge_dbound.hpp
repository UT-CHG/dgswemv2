...
void Problem::global_edge_distributed_kernel(const RKStepper& stepper, EdgeDistributedType& edge_dbound) {
    ...
    for (uint gp = 0; gp < edge_dbound.edge_data.get_ngp(); ++gp) {
        ...
        edge_internal.ze_avg_at_gp[gp] = (boundary.ze_at_gp[gp] + ze_ex) / 2.0;
        edge_internal.qx_avg_at_gp[gp] = (boundary.qx_at_gp[gp] + qx_ex) / 2.0;
        edge_internal.qy_avg_at_gp[gp] = (boundary.qy_at_gp[gp] + qy_ex) / 2.0;
    }

    edge_dbound.L2Projection(edge_internal.ze_avg_at_gp, edge_state.ze_hat);
    edge_dbound.L2Projection(edge_internal.qx_avg_at_gp, edge_state.qx_hat);
    edge_dbound.L2Projection(edge_internal.qy_avg_at_gp, edge_state.qy_hat);
    ...
    edge_dbound.ComputeUgp(edge_state.ze_hat, edge_internal.ze_hat_at_gp);
    edge_dbound.ComputeUgp(edge_state.qx_hat, edge_internal.qx_hat_at_gp);
    edge_dbound.ComputeUgp(edge_state.qy_hat, edge_internal.qy_hat_at_gp);
