...
void Problem::global_edge_distributed_kernel(const RKStepper& stepper, EdgeDistributedType& edge_dbound) {
    ...
    while (true) {
        ...
        for (uint dof = 0; dof < edge_dbound.edge_data.get_ndof_global(); ++dof) {
...
void Problem::global_edge_distributed_iteration(const RKStepper& stepper, EdgeDistributedType& edge_dbound) {
    ...
    for (uint dof_i = 0; dof_i < edge_dbound.edge_data.get_ndof_global(); ++dof_i) {
        for (uint dof_j = 0; dof_j < edge_dbound.edge_data.get_ndof_global(); ++dof_j) {
            Matrix<double, 3, 3> A =
