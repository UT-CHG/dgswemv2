...
inline void LLF_flux(const double ze_in,
                     const double ze_ex,
                     const double qx_in,
                     const double qx_ex,
                     const double qy_in,
    ...
    double max_eigenvalue =
        std::max(std::abs(un_in) + std::sqrt(Global::g * h_in), std::abs(un_ex) + std::sqrt(Global::g * h_ex));
    ...
    qx_flux = 0.5 * (un_in * qx_in + un_ex * qx_ex + (pe_in + pe_ex) * normal[GlobalCoord::x] +
