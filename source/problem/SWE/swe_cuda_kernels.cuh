...
struct state_args {
    state_args(SWE::State& state) :
            rhs_ze(state.rhs_ze.data()), rhs_qx(state.rhs_qx.data()), rhs_qy(state.rhs_qy.data()) {}
...
struct internal_args {
    internal_args(SWE::Internal& internal) :
            n_gp(internal.ze_at_gp.size()),
            ze_flux_at_gp{internal.ze_flux_at_gp[0].data(), internal.ze_flux_at_gp[1].data()},
            qx_flux_at_gp{internal.qx_flux_at_gp[0].data(), internal.qx_flux_at_gp[1].data()},
            qy_flux_at_gp{internal.qy_flux_at_gp[0].data(), internal.qy_flux_at_gp[1].data()},
            ze_at_gp(internal.ze_at_gp.data()),
            qx_at_gp(internal.qx_at_gp.data()),
            qy_at_gp(internal.qy_at_gp.data()),
            bath_at_gp(internal.bath_at_gp.data()),
            water_column_hgt_at_gp(internal.water_column_hgt_at_gp.data()) {}

    uint n_gp;
    double* ze_flux_at_gp[2];
    double* qx_flux_at_gp[2];
    double* qy_flux_at_gp[2];
    double* ze_at_gp;
    double* qx_at_gp;
    double* qy_at_gp;
    double* bath_at_gp;
    double* water_column_hgt_at_gp;
};

template <typename ElementType>
__global__ void cuda_volume_kernel1(internal_args internal, uint n_gp) {
...
__device__ double IntegrationDPhi(const uint dir, const uint dof, double* u_gp, const uint n_gp,
        const double*** int_fact_dphi) {
    double integral = 0;
    for (uint gp = 0; gp < n_gp; gp++) {
        integral += u_gp[gp] * int_fact_dphi[dof][dir][gp];
    }
    return integral;
}

template <typename ElementType>
__global__ void cuda_volume_kernel2(internal_args internal, state_args state,
        const double*** int_fact_dphi, uint n_dof) {
    uint dof = blockDim.x * blockIdx.x + threadIdx.x;
    if (dof < n_dof && dof > 0) {
        // skip dof = 0, which is a constant and thus trivially 0 NOT ALWAYS!
        state.rhs_ze[dof] = IntegrationDPhi(GlobalCoord::x, dof,
                internal.ze_flux_at_gp[GlobalCoord::x], internal.n_gp, int_fact_dphi) +
                IntegrationDPhi(GlobalCoord::y, dof,internal.ze_flux_at_gp[GlobalCoord::y],
                internal.n_gp, int_fact_dphi);
        state.rhs_qx[dof] = IntegrationDPhi(GlobalCoord::x, dof,
                internal.qx_flux_at_gp[GlobalCoord::x], internal.n_gp, int_fact_dphi) +
                IntegrationDPhi(GlobalCoord::y, dof, internal.qx_flux_at_gp[GlobalCoord::y],
                internal.n_gp, int_fact_dphi);
        state.rhs_qy[dof] = IntegrationDPhi(GlobalCoord::x, dof,
                internal.qy_flux_at_gp[GlobalCoord::x], internal.n_gp, int_fact_dphi) +
                IntegrationDPhi(GlobalCoord::y, dof, internal.qy_flux_at_gp[GlobalCoord::y],
                internal.n_gp, int_fact_dphi);
    }
}
