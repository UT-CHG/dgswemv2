...
void Problem::create_boundaries_kernel(
    ...
    // *** //
    using BoundaryTypes =
        Geometry::BoundaryTypeTuple<SWE::RKDG::Data, SWE::RKDG::BC::Land, SWE::RKDG::BC::Tide, SWE::RKDG::BC::Flow>;
    ...
    for (auto it = raw_boundaries.begin(); it != raw_boundaries.end(); it++) {
        if (it->first == SWE::BoundaryTypes::land) {
            ...
            while (itt != it->second.end()) {
                ...
                mesh.template CreateBoundary<BoundaryTypeTide>(raw_boundary, SWE::RKDG::BC::Tide(tide));
            ...
            while (itt != it->second.end()) {
                ...
                mesh.template CreateBoundary<BoundaryTypeFlow>(raw_boundary, SWE::RKDG::BC::Flow(flow));
