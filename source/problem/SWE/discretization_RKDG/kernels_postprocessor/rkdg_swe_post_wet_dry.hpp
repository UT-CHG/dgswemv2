...
void Problem::wetting_drying_kernel(const RKStepper& stepper, ElementType& elt) {
    ...
    if (h_avg <= PostProcessing::h_o + PostProcessing::h_o_threshold) {
        ...
        for (uint vrtx = 0; vrtx < elt.data.get_nvrtx(); vrtx++) {
            wd_state.q_at_vrtx[vrtx][SWE::Variables::ze] = h_avg - wd_state.bath_at_vrtx[vrtx];
        }
        ...
        if (n_dry_vrtx == 0) {
            ...
            for (uint vrtx = 0; vrtx < elt.data.get_nvrtx(); vrtx++) {
                if (wd_state.h_at_vrtx[vrtx] <= PostProcessing::h_o + PostProcessing::h_o_threshold) {
                    ...
                    del_qx += wd_state.q_at_vrtx[vrtx][SWE::Variables::qx];
                    del_qy += wd_state.q_at_vrtx[vrtx][SWE::Variables::qy];
            ...
            for (uint vrtx = 0; vrtx < elt.data.get_nvrtx(); vrtx++) {
                wd_state.q_at_vrtx[vrtx][SWE::Variables::ze] = wd_state.h_at_vrtx[vrtx] - wd_state.bath_at_vrtx[vrtx];

                if (wd_state.h_at_vrtx[vrtx] > PostProcessing::h_o + PostProcessing::h_o_threshold) {
                    wd_state.q_at_vrtx[vrtx][SWE::Variables::qx] += del_qx / (3 - n_dry_vrtx);
                    wd_state.q_at_vrtx[vrtx][SWE::Variables::qy] += del_qy / (3 - n_dry_vrtx);
                }
            }
        ...
        if (check_element) {
            ...
            double ze_h_max_vrtx = wd_state.q_at_vrtx[h_max_vrtx][SWE::Variables::ze];
