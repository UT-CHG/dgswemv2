...
namespace SWE {
void Problem::parse_source_data(const Stepper& stepper, ProblemMeshType& mesh, const ProblemInputType& problem_specific_input) {
    ...
    while (std::getline(meteo_file, line)) {
        ...
        if (!(input_string >> node_id >> meteo_data[0] >> meteo_data[1] >> meteo_data[2])) break;
...
void Problem::source_kernel(const Stepper& stepper, ElementType& elt) {
    ...
    if (wd_state.wet) {
        ...
        for (uint gp = 0; gp < elt.data.get_ngp_internal(); ++gp) {
            ...
            internal.qx_source_term_at_gp[gp] += Global::g * internal.h_at_gp[gp] * internal.dtidal_pot_at_gp[GlobalCoord::x][gp];
            internal.qy_source_term_at_gp[gp] += Global::g * internal.h_at_gp[gp] * internal.dtidal_pot_at_gp[GlobalCoord::y][gp];
        }
        }
