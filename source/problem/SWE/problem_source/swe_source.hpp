...
void get_source(const double t, ElementType& elt) {
    ...
    row(internal.source_at_gp, SWE::Variables::qx) =
        Global::g * vec_cw_mult(row(internal.dbath_at_gp, GlobalCoord::x), internal.q_at_gp[SWE::Variables::ze]);

    row(internal.source_at_gp, SWE::Variables::qy) =
        Global::g * vec_cw_mult(row(internal.dbath_at_gp, GlobalCoord::y), internal.q_at_gp[SWE::Variables::ze]);
