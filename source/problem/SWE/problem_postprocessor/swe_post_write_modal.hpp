...
void write_modal_data(const StepperType& stepper, MeshType& mesh, const std::string& output_path) {
    std::vector<std::pair<uint, std::array<DynView<double, SO::ColumnMajor>, SWE::n_variables>>> modal_q;
    std::vector<std::pair<uint, DynView<double, SO::ColumnMajor>>> modal_aux;
