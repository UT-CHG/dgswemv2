...
void Problem::init_interface_kernel(const StepperType& stepper, InterfaceType& intface) {
    if (stepper.GetOrder() == 2) {
        ...
        auto pe_in  = Global::g * (0.5 * vec_cw_mult(row(boundary_in.q_at_gp, SWE::Variables::ze),
                                                    row(boundary_in.q_at_gp, SWE::Variables::ze)) +
                                  vec_cw_mult(row(boundary_in.q_at_gp, SWE::Variables::ze),
        ...
        row(boundary_in.F_hat_at_gp, SWE::Variables::qx) =
            vec_cw_mult(uuh_in + pe_in, nx_in) + vec_cw_mult(uvh_in, ny_in);
        row(boundary_in.F_hat_at_gp, SWE::Variables::qy) =
            vec_cw_mult(uvh_in, nx_in) + vec_cw_mult(vvh_in + pe_in, ny_in);
        ...
        auto pe_ex  = Global::g * (0.5 * vec_cw_mult(row(boundary_ex.q_at_gp, SWE::Variables::ze),
                                                    row(boundary_ex.q_at_gp, SWE::Variables::ze)) +
                                  vec_cw_mult(row(boundary_ex.q_at_gp, SWE::Variables::ze),
        ...
        row(boundary_ex.F_hat_at_gp, SWE::Variables::qx) =
            vec_cw_mult(uuh_ex + pe_ex, nx_ex) + vec_cw_mult(uvh_ex, ny_ex);
        row(boundary_ex.F_hat_at_gp, SWE::Variables::qy) =
            vec_cw_mult(uvh_ex, nx_ex) + vec_cw_mult(vvh_ex + pe_ex, ny_ex);
    }
}
