...
bool Problem::ompi_solve_global_problem(std::vector<std::unique_ptr<OMPISimUnitType>>& sim_units) {
    ...
    for (uint su_id = 0; su_id < sim_units.size(); ++su_id) {
        ...
        sim_units[su_id]->discretization.mesh.CallForEachElement([&rhs_local_temp, &sparse_delta_local_inv](auto& elt) {
            auto& internal = elt.data.internal;

            uint ndof         = elt.data.get_ndof();
            uint local_offset = internal.local_dof_offset;

            subvector(rhs_local_temp, local_offset * SWE::n_variables, ndof * SWE::n_variables) = internal.rhs_local;
