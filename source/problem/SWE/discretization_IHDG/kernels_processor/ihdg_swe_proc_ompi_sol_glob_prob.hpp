...
bool Problem::ompi_solve_global_problem(std::vector<std::unique_ptr<OMPISimUnitType>>& sim_units) {
    ...
    delta_hat_global.resize(n_global_dofs, n_global_dofs);
    rhs_global.resize(n_global_dofs);
    ...
    for (uint su_id = 0; su_id < sim_units.size(); ++su_id) {
        ...
        delta_hat_global_temp.resize(n_global_dofs, n_global_dofs);
        rhs_global_temp.resize(n_global_dofs);
        ...
        sim_units[su_id]->discretization.mesh_skeleton.CallForEachEdgeInterface(
            [&rhs_global_temp, &sparse_delta_hat_global](auto& edge_int) {
                ...
                subvector(rhs_global_temp, global_dof_indx[0], ndof_global * SWE::n_variables) =
                    edge_internal.rhs_global;
