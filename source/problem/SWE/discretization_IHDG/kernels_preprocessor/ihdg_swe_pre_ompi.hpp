...
#pragma omp master
    {
        ...
        MPI_Gather(&total_local_dof_offset,
                   1,
                   MPI_UNSIGNED,
                   &total_local_dof_offsets.front(),
                   1,
                   MPI_UNSIGNED,
                   0,
                   MPI_COMM_WORLD);
        ...
        if (locality_id == 0) {
            // exclusive scan
            std::rotate(
                total_local_dof_offsets.begin(), total_local_dof_offsets.end() - 1, total_local_dof_offsets.end());
            std::rotate(
        ...
        MPI_Scatter(&total_local_dof_offsets.front(),
                    1,
                    MPI_UNSIGNED,
                    &total_local_dof_offset,
                    1,
                    MPI_UNSIGNED,
                    0,
                    MPI_COMM_WORLD);
        ...
        for (uint su_id = 0; su_id < sim_units.size(); ++su_id) {
            sim_units[su_id]->communicator.SendAll(SWE::CommTypes::preprocessor,
                                                   sim_units[su_id]->stepper.GetTimestamp());
        }
