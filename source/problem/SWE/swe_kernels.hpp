...
namespace SWE {
	...
	void Problem::source_kernel(const Stepper& stepper, ElementType& elt) {
		...
		auto source_ze = [t](Point<2>& pt) {
			return SWE::source_ze(t, pt);
		};

		auto source_qx = [t](Point<2>& pt) {
			return SWE::source_qx(t, pt);
		};

		auto source_qy = [t](Point<2>& pt) {
			return SWE::source_qy(t, pt);
		};
	...
	double Problem::compute_residual_L2_kernel(const Stepper& stepper, ElementType& elt) {
		...
		elt.ComputeFgp(true_ze, true_ze_gp);

		std::vector<double> sq_diff(elt.data.get_ngp_internal());
		for (uint gp = 0; gp < elt.data.get_ngp_internal(); gp++) {
			sq_diff[gp] = pow((true_ze_gp[gp] - est_ze_gp[gp]), 2);
		}
