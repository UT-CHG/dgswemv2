...
Inputs::Inputs(YAML::Node& gn_node) {
    ...
    if (YAML::Node sp_node = gn_node["spherical_projection"]) {
    ...
    if (YAML::Node wd_node = gn_node["wetting_drying"]) {
        if (wd_node["h_o"]) {
    ...
    if (YAML::Node sl_node = gn_node["slope_limiting"]) {
        if (sl_node["type"]) {
...
void Inputs::read_bcis(const std::string& bcis_file) {
    ...
    while (std::getline(file, line)) {
        ...
        } else if (btype == GN::BoundaryTypes::levee) {
