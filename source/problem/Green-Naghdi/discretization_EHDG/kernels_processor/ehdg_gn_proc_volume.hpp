    ...
    if (elt.data.wet_dry_state.wet /*&& elt.data.source.dispersive_correction*/) {
        ...
        for (uint dof_i = 0; dof_i < elt.data.get_ndof(); ++dof_i) {
            for (uint dof_j = 0; dof_j < elt.data.get_ndof(); ++dof_j) {
                internal.w1_w2(GN::n_dimensions * dof_i + GlobalCoord::x, dof_j) = elt.IntegrationPhiDPhi(
                    dof_i, GlobalCoord::x, dof_j, row(internal.w1_w2_kernel_at_gp, GlobalCoord::x));
                internal.w1_w2(GN::n_dimensions * dof_i + GlobalCoord::y, dof_j) = elt.IntegrationPhiDPhi(
                    dof_i, GlobalCoord::y, dof_j, row(internal.w1_w2_kernel_at_gp, GlobalCoord::y));
            }
        }
        ...
        for (uint dof_i = 0; dof_i < elt.data.get_ndof(); ++dof_i) {
            for (uint dof_j = 0; dof_j < elt.data.get_ndof(); ++dof_j) {
                ...
                internal.w2_w1(dof_i, GN::n_dimensions * dof_j + GlobalCoord::x) = elt.IntegrationPhiDPhi(
                    dof_j, GlobalCoord::x, dof_i, row(internal.w2_w1_kernel_at_gp, GlobalCoord::x));
                internal.w2_w1(dof_i, GN::n_dimensions * dof_j + GlobalCoord::y) = elt.IntegrationPhiDPhi(
                    dof_j, GlobalCoord::y, dof_i, row(internal.w2_w1_kernel_at_gp, GlobalCoord::y));
