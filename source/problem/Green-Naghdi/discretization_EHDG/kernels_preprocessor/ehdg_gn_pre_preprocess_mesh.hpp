#ifndef EHDG_GN_PRE_PREPROCESS_MESH_HPP
#define EHDG_GN_PRE_PREPROCESS_MESH_HPP
...
void Problem::preprocess_mesh_data(InputParameters<ProblemInputType>& input) {
    ...
    if (input.mesh_input.mesh_coordinate_sys == CoordinateSystem::spherical) {
        if (input.problem_input.spherical_projection.type == SWE::SphericalProjectionType::None) {
            std::cerr
