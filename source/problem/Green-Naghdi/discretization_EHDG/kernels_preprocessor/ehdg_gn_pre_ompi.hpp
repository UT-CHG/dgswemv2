...
void Problem::preprocessor_ompi(std::vector<std::unique_ptr<OMPISimUnitType>>& sim_units) {
    ...
    for (uint su_id = 0; su_id < sim_units.size(); ++su_id) {
        Problem::initialize_dc_data_parallel(sim_units[su_id]->discretization.mesh, sim_units[su_id]->problem_input);
    ...
    uint total_dc_global_dof_offset = std::accumulate(dc_global_dof_offsets.begin(), dc_global_dof_offsets.end(), 0);
    ...
    if (locality_id == 0) {
        n_dc_global_dofs = std::accumulate(total_dc_global_dof_offsets.begin(), total_dc_global_dof_offsets.end(), 0);
