#ifndef IHDG_GN_PRE_INIT_DATA_HPP
#define IHDG_GN_PRE_INIT_DATA_HPP
...
void Problem::initialize_data_serial(ProblemMeshType& mesh, const ProblemInputType& problem_specific_input) {
    mesh.CallForEachElement([&problem_specific_input](auto& elt) {
        ...
        if (problem_specific_input.initial_conditions.type == GN::InitialConditionsType::Constant ||
            ...
            HybMatrix<double, GN::n_variables> u_node(GN::n_variables, nnode);

            for (uint node_id = 0; node_id < nnode; ++node_id) {
                u_node(GN::Variables::ze, node_id) = problem_specific_input.initial_conditions.ze_initial;
                u_node(GN::Variables::qx, node_id) = problem_specific_input.initial_conditions.qx_initial;
                u_node(GN::Variables::qy, node_id) = problem_specific_input.initial_conditions.qy_initial;
            }

            state.q = elt.L2ProjectionNode(u_node);
        } else if (problem_specific_input.initial_conditions.type == GN::InitialConditionsType::Function) {
            auto u_init = [](Point<2>& pt) { return GN::ic_u(0, pt); };
    ...
    mesh.CallForEachInterface([&problem_specific_input](auto& intface) {
        ...
        row(boundary_in.aux_at_gp, GN::Auxiliaries::bath) = intface.ComputeNodalUgpIN(bathymetry);

        uint gp_ex;
        for (uint gp = 0; gp < ngp; ++gp) {
            gp_ex = ngp - gp - 1;

            boundary_ex.aux_at_gp(GN::Auxiliaries::bath, gp_ex) = boundary_in.aux_at_gp(GN::Auxiliaries::bath, gp);
        }
    });
    ...
    mesh.CallForEachBoundary([&problem_specific_input](auto& bound) {
        ...
        row(boundary.aux_at_gp, GN::Auxiliaries::bath) = bound.ComputeNodalUgp(bathymetry);
    });

    // SOURCE TERMS INITIALIZE
    // Manning N bottom friction
    if (problem_specific_input.bottom_friction.type == GN::BottomFrictionType::Manning) {
    ...
    // Coriolis effect
    if (problem_specific_input.coriolis.type == GN::CoriolisType::Enable) {
        mesh.CallForEachElement([&problem_specific_input](auto& elt) {
            double y_avg = 0;
            ...
            double latitude_avg = y_avg / Global::R_earth;
...
void Problem::initialize_data_parallel_pre_send(ProblemMeshType& mesh, const ProblemInputType& problem_specific_input) {
    ...
    mesh.CallForEachDistributedBoundary([&problem_specific_input](auto& dbound) {
        ...
        row(boundary.aux_at_gp, GN::Auxiliaries::bath) = dbound.ComputeNodalUgp(bathymetry);
    });
}

void Problem::initialize_data_parallel_post_receive(ProblemMeshType& mesh) {}
/* No post receive procedures in data initialization */
}
}
