...
std::vector<Point<2>> StraightTriangle::GetMidpointCoordinates(){
    ...
    for(uint midpt = 0; midpt < 3; midpt++) {
        midpoint_coord[midpt][GlobalCoord::x] = (
            this->nodal_coordinates[(midpt+1)%3][GlobalCoord::x] + 
            this->nodal_coordinates[(midpt+2)%3][GlobalCoord::x]
            )/2.0; 

        midpoint_coord[midpt][GlobalCoord::y] = (
            this->nodal_coordinates[(midpt+1)%3][GlobalCoord::y] + 
