...
void BASIS_TRI::Dubiner() {
	this->number_gp_area = this->integration_rule_area->GetNumberGP();
	this->number_gp_edge = this->integration_rule_line->GetNumberGP();
	...
	double* n1 = new double[this->number_gp_area];
	double* n2 = new double[this->number_gp_area];
	...
	for (int i = 0; i <= this->p; i++) {
		for (int j = 0; j <= this->p - i; j++) {
			this->phi_area[m] = new double[this->number_gp_area];
			this->dphi_dz1_area[m] = new double[this->number_gp_area];
			this->dphi_dz2_area[m] = new double[this->number_gp_area];

			dubiner_phi(i, j, this->number_gp_area, n1, n2, this->phi_area[m]);
			dubiner_dphi(i, j, this->number_gp_area, n1, n2, this->dphi_dz1_area[m], this->dphi_dz2_area[m]);
	...
	n1 = new double[this->number_gp_edge];
	n2 = new double[this->number_gp_edge];
	for (int i = 0; i < 3; i++) {
		if (i == 0) {
		...
		for (int j = 0; j <= this->p; j++) {
			for (int k = 0; k <= this->p - j; k++) {
				this->phi_edge[i][m] = new double[this->number_gp_edge];

				dubiner_phi(j, k, this->number_gp_edge, n1, n2, this->phi_edge[i][m]);
	...
	*/
	...
	double* w = new double[this->number_gp_area];
	for (int i = 0; i < this->number_gp_area; i++) {
		w[i] = this->integration_rule_area->GetWeight(i);
	}

	dubiner_test(this->p, this->number_gp_area, phi_area, w);
