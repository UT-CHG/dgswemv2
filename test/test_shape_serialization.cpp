...
bool is_equal(Shape::StraightTriangle& o_tri, Shape::StraightTriangle& i_tri) {
    ...
    for (uint bd_id = 0; bd_id < 3; ++bd_id) {
        DynVector<Point<2>> test_pt{test_points[bd_id]};

        DynVector<StatVector<double, 2>> o_normal = o_tri.GetSurfaceNormal(bd_id, test_pt);
        DynVector<StatVector<double, 2>> i_normal = i_tri.GetSurfaceNormal(bd_id, test_pt);
...
int main() {
    DynVector<Point<3>> nodal_coord(3);
    nodal_coord[0] = {-1, 0, 0};
    nodal_coord[1] = {1, 0, 0};
    nodal_coord[2] = {0, 1, 0};
