...
int main() {
  ...
  for ( auto& pair : rk_pairs ) {
	...
	for ( uint s = 0; s < stage+1; ++ s ) {
	  ...
	  y[stage+1][0] += rk_stepper.ark[stage][s]*y[s][0]
	    + dt*rk_stepper.brk[stage][s]*rhs[s][0];

	  y[stage+1][1] += rk_stepper.ark[stage][s]*y[s][1]
	    + dt*rk_stepper.brk[stage][s]*rhs[s][1];
	}
	++rk_stepper;
      }
    ...
    std::cout << "At time: " << t << "\n";
    std::cout << "Got: " << std::setprecision(14)
	      << y[0][0] << " Should be: " << std::sin(t) + 0.5*t << "\n";
