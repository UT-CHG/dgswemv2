        ...
        for (auto& sm : loc) {
            for (auto& e : sm._elements) {
                for (uint k = 0; k < e.second.neighbor_ID.size(); ++k) {
                    if (e.second.boundary_type[k] == INTERNAL && !sm._elements.count(e.second.neighbor_ID[k])) {
                        error_found = true;
                        std::cerr << "Element " << e.first << " cannot find internal neighbor of id "
                                  << e.second.neighbor_ID[k] << '\n';
                    } else if (e.second.boundary_type[k] == DISTRIBUTED &&
                               sm._elements.count(e.second.neighbor_ID[k])) {
                        error_found = true;
