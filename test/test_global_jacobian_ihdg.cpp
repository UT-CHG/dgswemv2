...
int main(int argc, char* argv[]) {
    ...
    SparseMatrix<double> _delta_hat_global;
    DynVector<double> _rhs_global;
    ...
    _delta_hat_global.resize(dof_global, dof_global);
    _rhs_global.resize(dof_global);
    ...
    delta_hat_global = delta_hat_global - delta_global * delta_local_inv * delta_hat_local;

    rhs_global = rhs_global - delta_global * delta_local_inv * rhs_local;

    // store for future
    rhs_global_prev = rhs_global;

    // estimate diff
    delta_global_diff_est = delta_hat_global * delta_q_hat;

    // set del_q
    rhs_global = delta_q_hat;
    ...
    mesh.CallForEachElement([&](auto& elt) {
        ...
        auto rhs_local_ref = subvector(rhs_local, elt_ID * 9, 9);
    ...
    mesh_skeleton.CallForEachEdgeInterface([&](auto& edge_int) {
        ...
        auto rhs_global_ref = subvector(rhs_global, edg_ID * 6, 6);
    ...
    for (uint dof = 0; dof < dof_global; dof++) {
            if (!Utilities::almost_equal(
                    delta_global_diff_est[dof], rhs_global_prev[dof], 1.0e12)) {
                std::cerr << "error del hat" << std::endl;
                std::cout << std::setprecision(15) << delta_global_diff_est[dof] << ' '
                          << rhs_global_prev[dof] << std::endl;
            }
    }
}