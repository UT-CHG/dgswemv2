...
bool check_equality(const SWE::RKDG::State& o_state, const SWE::RKDG::State& i_state) {
    return double_vectors_are_same(o_state.ze, i_state.ze) && double_vectors_are_same(o_state.qx, i_state.qx) &&
           double_vectors_are_same(o_state.qy, i_state.qy) && double_vectors_are_same(o_state.bath, i_state.bath) &&
...
SWE::RKDG::Spherical build_spherical(const uint nnode,
                                     const uint nbound,
                                     const uint ngp_internal,
                                     const std::vector<uint>& ngp_boundary,
                                     double& counter) {
...
bool check_equality(const SWE::RKDG::Spherical& o_sp, const SWE::RKDG::Spherical& i_sp) {
    ...
    for (uint i = 0; i < o_sp.sp_at_gp_boundary.size(); ++i) {
        are_same &= double_vectors_are_same(o_sp.sp_at_gp_boundary[i], i_sp.sp_at_gp_boundary[i]);
    }
...
bool check_equality(const SWE::RKDG::Source& o_src, const SWE::RKDG::Source& i_src) {
    return almost_equal(o_src.coriolis_f, i_src.coriolis_f) && (o_src.manning == i_src.manning) &&
           (o_src.coriolis_f == i_src.coriolis_f) && almost_equal(o_src.coriolis_f, i_src.coriolis_f) &&
...
bool check_equality(const SWE::RKDG::WetDry& o_wd, const SWE::RKDG::WetDry& i_wd) {
    return double_vectors_are_same(o_wd.ze_lin, i_wd.ze_lin) && double_vectors_are_same(o_wd.qx_lin, i_wd.qx_lin) &&
           ...
           almost_equal(o_wd.bath_min, i_wd.bath_min) && (o_wd.went_completely_dry == i_wd.went_completely_dry) &&
           (o_wd.wet == i_wd.wet);
}
...
bool check_equality(const SWE::Data& o_data, const SWE::Data& i_data) {
    ...
    return (o_data.get_nnode() == i_data.get_nnode()) && (o_data.get_nvrtx() == i_data.get_nvrtx()) &&
           (o_data.get_ndof() == i_data.get_ndof()) && (o_data.get_ngp_internal() == i_data.get_ngp_internal()) &&
           check_equality(o_data.spherical_projection, i_data.spherical_projection) &&
           check_equality(o_data.source, i_data.source) && check_equality(o_data.wet_dry_state, i_data.wet_dry_state) &&
           check_equality(o_data.slope_limit_state, i_data.slope_limit_state) &&
           check_equality(o_data.internal, i_data.internal) && is_equal;
}
