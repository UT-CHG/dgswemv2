...
const static auto compare = [](AdcircFormat meshA, AdcircFormat meshB)->bool
{
  if (
      ...
      (meshA.nodes == meshB.nodes) &&
      (meshA.elements == meshB.elements) &&
      (meshA.NOPE == meshB.NOPE) &&
      (meshA.NETA == meshB.NETA) &&
      (meshA.NBDV == meshB.NBDV) &&
      (meshA.IBTYPE == meshB.IBTYPE) &&
      (meshA.NBOU == meshB.NBOU) &&
      (meshA.NVEL == meshB.NVEL) &&
      (meshA.IBTYPE == meshB.IBTYPE) &&
      (meshA.NBVV == meshB.NBVV)
       ) { return true; }
