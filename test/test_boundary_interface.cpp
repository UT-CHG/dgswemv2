...
int main() {
	...
	ElementType triangle(0, master, vrtxs, 
		std::vector<uint>{ DEFAULT_ID, DEFAULT_ID, DEFAULT_ID },
		std::vector<unsigned char>{ SWE::BoundaryConditions::land, SWE::BoundaryConditions::land, SWE::BoundaryConditions::land });
	...
	for (uint dof = 0; dof < 66; dof++) {
		...
		for (uint n_bound = 0; n_bound < 3; n_bound++) {
			...
			if (!almost_equal(boundaries[n_bound].IntegrationPhi(dof, unit), boundaries[n_bound].IntegrationPhi(0, gp_vals), 1.e+03)) {
				error_found = true;
	...
	for (uint dof = 0; dof < 66; dof++) {
		...
		for (uint n_intface = 0; n_intface < 3; n_intface++) {
			...
			if (!almost_equal(interfaces[n_intface].IntegrationPhiIN(dof, unit), interfaces[n_intface].IntegrationPhiIN(0, gp_vals), 1.e+03)) {
				error_found = true;
			...
			if (!almost_equal(interfaces[n_intface].IntegrationPhiEX(dof, unit), interfaces[n_intface].IntegrationPhiEX(0, gp_vals), 1.e+03)) {
				error_found = true;
