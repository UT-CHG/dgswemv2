...
std::vector<double> jacobi_polynomial(int n, int a, int b, const std::vector<double>& x) { 
	...
	if (n == 0) {
        for (int i = 0; i < n_pts; i++) { P[i] = 1; }
    }
    else if (n == 1) {
        for (int i = 0; i < n_pts; i++) { P[i] = (a - b + (a + b + 2)*x[i]) / 2; }
    }
    else {
        Array2D<double> J(3);
        J[0].reserve(n_pts);
...
std::vector<double> jacobi_polynomial_derivative(int n, int a, int b, const std::vector<double>& x) {
	...
	if (n == 0) {
        for (int i = 0; i < n_pts; i++) { dP[i] = 0; }
    }
    else if (n == 1) {
        for (int i = 0; i < n_pts; i++) { dP[i] = (a + b + n + 1) / 2.0; }
    }
    else {
        dP = jacobi_polynomial(n - 1, a + 1, b + 1, x);

        for (int i = 0; i < n_pts; i++) { dP[i] = dP[i] * (a + b + n + 1) / 2.0; }
    }
