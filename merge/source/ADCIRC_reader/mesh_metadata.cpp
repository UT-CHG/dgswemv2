...
MeshMetaData::MeshMetaData(const AdcircFormat& mesh_file)
{
 ...
 for ( const auto& elt : mesh_file.elements ) {
   ...
   for ( uint i = 0; i < 3; ++i ) {
     ...
     _meta[ID].coordinates[i][0] = tmp[0];
     _meta[ID].coordinates[i][1] = tmp[1];
   }
  }
  ...
  {//make all edges
    ...
    for ( const auto& elt : mesh_file.elements ) {
      ...
      for ( int k = 0; k < 3; ++k ) {
	std::uint64_t curr_key =  (static_cast<std::uint>(std::min(node[k],node[(k+1)%3])))<<32
	  | static_cast<std::uint>(std::max(node[k],node[(k+1)%3]));
	...
	_meta[eltA_id].boundary_type[fidA] = SWE::BoundaryConditions::internal;
	_meta[eltA_id].neighbor_IDs[fidA] = eltB_id;

	_meta[eltB_id].boundary_type[fidB] = SWE::BoundaryConditions::internal;
	_meta[eltB_id].neighbor_IDs[fidB] = eltA_id;
	...
	_meta[elt_id].boundary_type[fid] = mesh_file.get_ibtype( nodes );
      }
    }
  }
}
