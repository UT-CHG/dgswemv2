 ...
 for ( const auto& elt : mesh_file.elements ) {
  ...
  {//make all edges
    ...
    for ( const auto& elt : mesh_file.elements ) {
      ...
      for ( int k = 0; k < 3; ++k ) {
	std::uint64_t curr_key = (std::uint64_t) (std::min(node[k],node[(k+1)%3]))<<32
	...
	std::array<int,2> nodes { mesh_file.elements.at(elt_id)[fid],
	    mesh_file.elements.at(elt_id)[ (fid+1) %3] };
