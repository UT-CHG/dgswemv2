...
AdcircFormat::AdcircFormat(const char* fort14)
{
  ...
  {//process open boundaries
    ifs >> NOPE;                 ifs.ignore(1000,'\n');
    ifs >> NETA;                 ifs.ignore(1000,'\n');

    NBDV.reserve(NOPE);
    int n_nodes_bdry;
    for ( int bdry = 0; bdry < NOPE; ++bdry ) {
      ifs >> n_nodes_bdry;       ifs.ignore(1000,'\n');
      NBDV.push_back(std::vector<int>(n_nodes_bdry));

      for ( int n = 0; n < n_nodes_bdry; ++n ) {
        ifs >> NBDV[bdry][n];    ifs.ignore(1000,'\n');
      }
    }
  }

  {//process land boundaries
    ifs >> NBOU;       ifs.ignore(1000,'\n');
    ifs >> NVEL;       ifs.ignore(1000,'\n');
    ...
    for ( int bdry = 0; bdry < NBOU; ++bdry ) {
      ...
      for ( int n = 0; n < n_nodes_bdry; ++n ) {
        ifs >> NBVV[bdry][n];     ifs.ignore(1000,'\n');
      }
    }
  }
